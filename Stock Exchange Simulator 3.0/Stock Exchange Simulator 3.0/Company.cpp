...
Company::Company(int id, string name, double cash,int paytime, int div_period, double initial_dividend, double initial_mu, double sigma, double alpha, double beta, double warning)
{
    _id = id;
    _name = name;
...
{
    unsigned long n = _cash.size();
    cout << "id: " << _id << "\n";
    cout << "time: " << n-2 << "\n";
    cout << "Cash: $" << _cash[n-2] << ". Result: $" << _results[n-2] << "(Expected : $" << _mu[n-2]*_div_period << ")\n";
    cout << "Objective: $" << _objectives[n-2] << ". Investment: $" << _investments[n-2] << "\n";
    cout << "Savings: $" << _savings[n-2] << ". Dividends: $" << _dividends[n-2] << "\n";
...
void Company::UpdateCompany(int time, double rate, int nb_stock, vector<Agent*> pagents)
{
    ...
    if (time == _paytime)
    {
        ...
        double div = convert_price(_dividends[n-1]/nb_stock);
        if (div>0)
        {
